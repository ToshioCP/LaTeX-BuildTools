...
このことによって、本文のファイルがchapやpartをまたいで移動してもコマンドのパス名を変更しなくてすむ。
\\inputと\\incudegraphicsのパス名の調整はプログラムが自動で行ってくれる。
...
前処理は自動で行われるので、ユーザはsec1.mdのように拡張子をmdにしておくだけでよい。
...
ruby2.7で開発したが、2.0以降のrubyならば動くのではないかと思う。
...
でインストールされる。
実行ファイルが/usr/local/binに、ライブラリ（lib_latex_utils）が$LOAD_PATH[0]にセットされる。
$LOAD_PATH[0]は/usr/local/lib/site_ruby/2.7.0などで、システムによって異なる。
rbenvを使ってrubyをインストールした場合は、ユーザのホームディレクトリの.rbenvディレクトリ以下にライブラリ保存先がある。
